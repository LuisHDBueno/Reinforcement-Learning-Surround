...
class MCTS():
    ...
    def verify_chosen_action(self, board, action) -> bool:
        ...
        ver_board = self.game.player2.move(ver_board, self.adversary.play(board))
    ...
    def grow_tree(self) -> None:
        ...
        reward, *_ = self.game.step((action, self.adversary.play(self.game.board)))

        if (not (self.game.lose1 or self.game.lose2)):
            node[chosen_action] = self.grow_tree()
            node[forbiden_move] = node[self.game.player1.last_action]
            return node
        else:
    ...
    def tree_to_list(self) -> list:
        ...
        while type(node["board"]) == type(np.array([1])):
            boards_buffer.append(node["board"])
            node_probs = np.zeros((4,), dtype=np.float32)
            action_space.remove(node["forbiden_move"])
            for action in action_space:
                node_probs[action] = self.get_node_reward(node[action])
    ...
    def get_buffers(self):
        ...
        boards_buffer, probs_buffer = self.tree_to_list()
